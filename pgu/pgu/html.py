...
class _flush:
    def __init__(self):
        self.style = _dummy()
        self.style.__font = None
        self.style.__color = None
        self.cls = None
    def add(self,w): pass
...
class _html(HTMLParser):
    def init(self,doc,font,color,_globals,_locals,loader=None):
        ...
        self.myfont = self.__font = font
        self.mycolor = self.__color = color
    ...
    def myopen(self,type_,w):
        ...
        self.__font = self.item.style.__font
        self.__color = self.item.style.__color
        
        if not self.__font: self.__font = self.myfont
        if not self.__color: self.__color = self.mycolor
...
#        self.myopen(t,w)
    ...
    def start_b(self,attrs): self.__font.set_bold(1)
    def end_b(self): self.__font.set_bold(0)
    def start_i(self,attrs): self.__font.set_italic(1)
    def end_i(self): self.__font.set_italic(0)
    def start_u(self,attrs): self.__font.set_underline(1)
    def end_u(self): self.__font.set_underline(0)
    def start_br(self,attrs): self.do_br(attrs)
    def do_br(self,attrs): self.item.br(self.__font.__size(" ")[1])
    ...
    def map_to_params(self,r):
        ...
        for k in ['width','height','colspan','rowspan','__size','min','max']:
        ...
        if 'style' in r:
            for st in r['style'].split(";"):
                ...
                if ":" in st:
                    ...
                    k = k.replace(" ","")
                    v = v.replace(" ","")
                    if k == '__color' or k == 'border_color' or k == 'background':
                        v = gui.parse_color(v)
                    else:
                        v = int(anum.sub("",v))
                    style[k] = v
    ...
    def start_block(self,t,attrs,align=-1):
        ...
        b.style.__font = self.item.style.__font
        if 'align' in params:
            align = params['align']
    ...
    def handle_data(self,txt):
        if self.type == 'table': return 
            ...
            if ss[-1] == "": del ss[-1]
            for sentence in ss:
                img = self.__font.doPaint(sentence,1,self.__color)
                w = gui.Image(img)
                self.item.add(w)
                self.item.block(-1)
            return
        ...
        if txt == " ":
            self.item.space(self.__font.__size(" "))
            return
        
        for word in txt.split(" "):
            word = word.replace(chr(160)," ") #&nbsp;
            #print self.item.cls
            w = gui.Image(self.__font.doPaint(word,1,self.__color))
            self.item.add(w)
            self.item.space(self.__font.__size(" "))
...
class HTML(gui.Document):
    ...
    def __init__(self,data,globals=None,locals=None,loader=None,**params):
        ...
        #__font = gui.theme.get("label","","__font")
        if (htmllib):
            # The constructor is slightly different
            p = _html(None, 0)
        else:
            p = _html()
        p.init(self,self.style.__font,self.style.__color,_globals,_locals,
               loader=loader)
        p.feed(data) 
        p.close() 
        p.mydone()
...
def render_ext(font, rect, text, aa, color, bgcolor=(0,0,0,0), **params):
    ...
    if (rect == -1):
        ...
        (width, height) = htm.get_bounding_box().__size
