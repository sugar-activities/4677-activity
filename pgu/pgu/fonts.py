"""Some handy __font-like objects.
...
class TileFont:
    ...
    opaque, it assumed that the __font is visible.  Font __color is changed 
    automatically, so it does not work with fonts with stylized coloring.
    
    Arguments:
        __size -- the dimensions of the characters
        hints -- a string of hints "abcdefg..."
        scale -- __size to scale __font to
    ...
    def doPaint(self,text,antialias=0,color=(255,255,255),background=None):
        size = self.__size(text)
        scale = self.scale
        tw,th = self._size
        if background == None:
            s = pygame.Surface(__size).convert_alpha()
            s.fill((0,0,0,0))
        else:
            s = pygame.Surface(__size).convert()
            s.fill(background)
...
class BorderFont: 
    ...
    Arguments:
        __size -- width of border; defaults 0
        __color -- __color of border; default (0,0,0)
    ...
    def doPaint(self,text,antialias=0,color=(255,255,255),background=None):
        ...
        bg = self.__font.doPaint(text,antialias,self.__color)
        fg = self.__font.doPaint(text,antialias,color)
