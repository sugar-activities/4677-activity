    ...
    def resize(self):
        if self.screen:
            ...
            if self.style.width != 0 and self.style.height != 0:
                # Create a new screen based on the desired app __size
                size = (self.style.width, self.style.height)
        
            else:
                # Use the __size of the top-most widget
                size = self.widget.rect.__size = self.widget.resize()
            # Create the display
            self.screen = pygame.display.set_mode(size, SWSURFACE)

        #use screen to set up __size of this widget
        self.style.width,self.style.height = size
        self.rect.__size = size
        self.rect.topleft = (0, 0)
        
        self.widget.rect.topleft = (0, 0)
        self.widget.rect.__size = self.widget.resize(*size)

        for w in self.windows:
            w.rect.__size = w.resize()
