...
class Theme:
    ...
    def _get(self, cls, pcls, attr):
        ...
        if (os.path.splitext(vals[0].lower())[1] in self.image_extensions):
            ...
            # This is a __font value
            name = vals[0]
            size = int(vals[1])
            if (name.endswith(".ttf")):
                # Load the __font from a file
                v = pygame.__font.Font(os.path.join(dname, name), size)
            else:
                # Must be a system __font
                v = pygame.__font.SysFont(name, size)
    ...
    def getstyle(self, cls, pcls, attr):
        ...
        Arguments:
            ...
            attr -- attribute, for example "image", "background", "__font", "color", etc.
...
#                 s = w._theme_paint_bkgr.convert()
    ...
    def doPaint(self,surf,box,r,size=None,offset=None):
        """Renders a box using an image.

        Arguments:
            surf -- the target pygame displaySurface
            box -- pygame displaySurface or color
            r -- pygame rect describing the size of the image to doPaint

        If 'box' is a displaySurface, it is interpreted as a 3x3 grid of tiles. The 
...
class Background(widget.Widget):
    ...
    def paint(self, s, size=None, offset=None):
        ...
        self.theme.doPaint(s,v,r, size=size, offset=offset)
