...
class Input(widget.Widget):
    ...
    Example:
        w = Input(value="Cuzco the Goat",__size=20)
        w = Input("Marbles")
    ...
    def __init__(self,value="",size=20,**params):
        ...
        Keyword arguments:
            value -- initial text
            __size -- __size for the text box, in characters
    ...
    def paint(self,s):
        ...
        w,h = self.__font.__size(self.value[0:self.pos])
        x = w-self.vpos
        if x < 0: self.vpos -= -x
        if x+cs > s.get_width(): self.vpos += x+cs-s.get_width()
        
        s.blit(self.__font.doPaint(self.value, 1, self.style.__color),(-self.vpos,0))
...
class Password(Input):
    ...
    def paint(self,s):
        ...
        w,h = self.__font.__size(show)
        x = w-self.vpos
        if x < 0: self.vpos -= -x
        if x+cs > s.get_width(): self.vpos += x+cs-s.get_width()
        
        s.blit(self.__font.doPaint(show, 1, self.style.__color),(-self.vpos,0))
        
        if self.container.myfocus is self:
            #w,h = self.__font.__size(self.value[0:self.pos])            
            w,h = self.__font.__size(show[0:self.pos])
            r.x = w-self.vpos
            r.w = cs
            r.h = h
            s.fill(self.style.__color,r)
