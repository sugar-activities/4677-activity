...
# Turns a descriptive string or a tuple into a pygame __color
...
# Determines if the given object is a pygame-compatible __color or not
def is_color(col):
    # In every version of pygame (up to 1.8.1 so far) will interpret
    # a tuple as a __color.
...
class Color(widget.Widget):
    """A widget that renders as a solid block of __color.
    
    Note the __color can be changed by setting the 'value' field, and the 
...
class Label(widget.Widget):
    ...
    def __init__(self, value="", **params):
        ...
        widget.Widget.__init__(self, **params)
        self.style.check("__font")
        self.value = value
        self.__font = self.style.__font
        self.style.width, self.style.height = self.__font.__size(self.value)
    
    def paint(self,s):
        """Renders the label onto the given surface in the upper-left corner."""
        s.blit(self.__font.doPaint(self.value, 1, self.style.__color),(0,0))
    ...
    def set_font(self, font):
        """Set the __font used to doPaint this label."""
        self.__font = font
        # Signal to the application that we need a resize
        self.chsize()

    def resize(self,width=None,height=None):
        # Calculate the __size of the rendered text
        (self.style.width, self.style.height) = self.__font.__size(self.value)
