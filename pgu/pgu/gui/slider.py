from . import basic, pguglobals, table, widget
from .const import *
...
class _slider(widget.Widget):
    ...
    def __init__(self, value, orient, min, max, size, step=1, **params):
        ...
        self.min, self.max, self.value, self.orient, self.__size, self.step = min, max, value, orient, size, step
    ...
    def paint(self, s):
        ...
        if self.orient == _SLIDER_HORIZONTAL:
            r.x = (self.value - self.min) * (r.w - self.__size) / max(1, self.max - self.min);
            r.w = self.__size;
        else:
            r.y = (self.value - self.min) * (r.h - self.__size) / max(1, self.max - self.min);
            r.h = self.__size;
            
        self.bar = r

        pguglobals.app.theme.doPaint(s, self.style.bar, r)
    ...
    def event(self, e):
        ...
        if adj:
            if self.orient == _SLIDER_HORIZONTAL:
                d = self.__size / 2 - (r.w / (self.max - self.min + 1)) / 2
                self.value = (x - d) * (self.max - self.min) / (r.w - self.__size + 1) + self.min
            else:
                d = self.__size / 2 - (r.h / (self.max - self.min + 1)) / 2
                self.value = (y - d) * (self.max - self.min) / (r.h - self.__size + 1) + self.min
    ...
    def __setattr__(self, k, v):
        ...
        if k == 'value' and _v != NOATTR and _v != v: 
            self.send(CHANGE)
            self.repaint()
        if hasattr(self, '__size'):
            sz = min(self.__size, max(self.style.width, self.style.height))
            sz = max(sz, min(self.style.width, self.style.height))
            self.__dict__['__size'] = sz
            # self.__size = sz
...
class VSlider(_slider):
    ...
    def __init__(self, value, min, max, size, step=1, **params):
        ...
        Arguments:
            value -- the default positionXY of the slider, between min and max
            min -- the minimum value for the slider
            max -- the maximum value
            __size -- the length of the slider bar in pixels
...
class HScrollBar(table.Table):
    ...
    def resize(self, width=None, height=None):
        ...
        setattr(self.slider, '__size', self.__size * self.slider.style.width / max(1, self.style.width))
        # self.slider.__size = self.__size * self.slider.style.width / max(1,self.style.width)
...
class VScrollBar(table.Table):
    ...
    def resize(self, width=None, height=None):
        ...
        setattr(self.slider, '__size', self.__size * self.slider.style.height / max(1, self.style.height))
        return table.Table.resize(self, width, height)
