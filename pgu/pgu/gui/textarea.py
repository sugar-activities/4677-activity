...
class TextArea(widget.Widget):
    ...
    Example:
        w = TextArea(value="Cuzco the Goat",__size=20)
        w = TextArea("Marbles")
        w = TextArea("Groucho\nHarpo\nChico\nGummo\nZeppo\n\nMarx", 200, 400, 12)
    ...
    def __init__(self,value="",width = 120, height = 30, size=20,**params):
        ...
        self.__font = self.style.__font        # The __font used for rendering the text
        self.cursor_w = 2                 # Cursor width (NOTE: should be in a style)
        w,h = self.__font.__size("e"*size)    
...
##        return self.rect.w, self.rect.h
    ...
    def paint(self,s):
        ...
        # TODO1: What's up with this 20 magic number? It's the margin of the left and right sides, but I'm not sure how this should be gotten other than by trial and error.
    ...
    # Returns a rectangle that is of the __size and position of where the cursor is drawn    
    def getCursorRect(self):
        lw = 0
        if (len(self.lines) > 0):
            lw, lh = self.__font.__size( self.lines[ self.vpos ][ 0:self.hpos ] )
    ...
    def setCursorByXY(self, pos):
        ...
        for cnt in range(0, len(currentLine) ):
            self.hpos = cnt
            lw, lh = self.__font.__size( currentLine[ 0:self.hpos + 1 ] )
            if (lw > x):
                break
        
        lw, lh = self.__font.__size( currentLine )
        if (lw < x):
    ...
    def doLines(self, max_line_w):
        ...
        while inx >= 0:
            ...
            lw, self.line_h = self.__font.__size( self.value[ line_start : inx ] )
            ...
            if (lw > max_line_w):
                ...
                # TODO1: Check for extra-long words here that exceed the length of a line, to wrap mid-word
