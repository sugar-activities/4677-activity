...
class Tilevid(Vid):
    ...
    def paint(self,s):
        ...
        ox,oy = self.__view.x,self.__view.y
        tlayer = self.tlayer
        blayer = self.blayer
        alayer = self.alayer
        ...
        self.updates = []
        self._view = pygame.Rect(self.__view)
    ...
    def update(self,s):
        ...
        if self.bounds != None: self.__view.clamp_ip(self.bounds)
        if self.__view.x != self._view.x or self.__view.y != self._view.y: 
            return self.paint(s)
    ...
    def screen_to_tile(self,pos):
        x,y = pos
        x,y = x+self.__view.x,y+self.__view.y
