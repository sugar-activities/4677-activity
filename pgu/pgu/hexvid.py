...
class Hexvid(Vid):
    ...
    def paint(self,screen):
        ...
        self.__view.w,self.__view.h = sw,sh
        ...
        view = self.__view
        adj = self.adj = pygame.Rect(-self.__view.x,-self.__view.y,0,0)
        ...
        if self.bounds != None: self.__view.clamp_ip(self.bounds)

        ox,oy = self.screen_to_tile((0,0))
        sx,sy = self.tile_to_view((ox,oy))
        dx,dy = sx - self.__view.x,sy - self.__view.y
        ...
        for i2 in xrange(-bot,self.__view.h/tile_h2+bot*3): #NOTE: 3 seems a bit much, but it works.
            ...
            x -= tile_w/2
            for i1 in xrange(-1,self.__view.w/tile_wi+1):
    ...
    def screen_to_tile(self,pos): #NOTE HACK : not sure if the 3/8 is right or not, but it is pretty close...
        pos = pos[0]+self.__view.x + self.tile_w*3/8,pos[1]+self.__view.y
        pos = self.view_to_tile(pos)
        return pos
    
    def tile_to_screen(self,pos):
        pos = self.tile_to_view(pos)
        pos = pos[0]-self.__view.x,pos[1]-self.__view.y
        return pos
